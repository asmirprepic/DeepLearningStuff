...
def preprocess_and_create_sequences(df, seq_length=60):
    ...
    for i in range(len(data_scaled) - seq_length):
        ...
        y = df.iloc[i + seq_length].values[0]  # Predicting the first numerical feature in original scale
        xs.append(x)
        ys.append(y)
...
def build_model(input_shape):
    model = models.Sequential([
        ...
        layers.LSTM(32),
        layers.Dense(1, activation='softplus')  # Softplus activation to ensure non-negative output
    ])
...
def make_prediction(model, scaler, data, seq_length):
    ...
    predicted_value_original_scale = np.exp(predicted_value[0][0])  # Convert from log scale

    print(f"Predicted next count_0 value: {predicted_value_original_scale}")

    plt.figure(figsize=(10, 6))
    plt.plot(data['count_0'].values, label='Actual Data')
    plt.plot(range(len(data), len(data) + 1), [predicted_value_original_scale], 'ro', label='Predicted Value')
    plt.xlabel('Time')
    plt.ylabel('count_0 Value')
    plt.legend()
    plt.title('count_0 Time Series and Prediction')
    plt.show()
